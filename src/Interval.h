...
public:
	...
	Interval<T> Inverse();
	Interval<T> Absolute();
	Interval<T> Scale(unsigned int scale);
	T GetWidth();
...
template<typename T>
inline Interval<T> Interval<T>::operator ^(int y) {
	Interval<T> x(this->a, this->b);
	Interval<T> r = {0, 0};
	return IPow(x, y);
}
...
template<typename T>
inline Interval<T> operator ^(Interval<T> x, int y) {
	return IPow(x, y);
}

template<typename T>
inline Interval<T> Interval<T>::Scale(unsigned int scale) {
	Interval<T> r(this->a, this->b);
...
// author why
template<typename T>
Interval<T> IPow(const Interval<T>& x, int l) {
	Interval<T> r;
	fesetround(FE_DOWNWARD);
	r.a = pow(x.a, l);
	fesetround(FE_UPWARD);
...
typedef interval_arithmetic::Interval<long double> I;
